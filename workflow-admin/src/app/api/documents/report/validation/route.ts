...
export async function GET(request: NextRequest) {
  try {
    ...
    if (searchParams.get('validatorId')) params.append('validatorId', searchParams.get('validatorId')!);

    const queryString = params.toString();
    const url = `${GATEWAY_URL}/documents/report/validation${queryString ? `?${queryString}` : ''}`;
